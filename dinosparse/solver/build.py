...
class GradientClipType(Enum):
    VALUE = "value"
    NORM = "norm"
    FULL_MODEL = "full_model"   # 全模型梯度裁剪：所有参数拼成一个向量算全局范数再裁剪（DETR/SparseR-CNN 推荐）
...
def _create_gradient_clipper(cfg: CfgNode) -> _GradientClipper:
    ...
    # FULL_MODEL 复用 clip_grad_norm（范数裁剪），区别在于作用范围（per-group vs 全模型），
    # 范围由 maybe_add_gradient_clipping 里选 per_param_clipper 还是 global_clipper 决定。
...
def maybe_add_gradient_clipping(
    ...
    支持三种裁剪类型（cfg.SOLVER.CLIP_GRADIENTS.CLIP_TYPE）：
      - "value"       ：per-param，按值裁剪（detectron2 原生）。
      - "norm"        ：per-param-group，按范数裁剪（detectron2 原生）。
      - "full_model"  ：全模型裁剪——把所有 param_group 的梯度拼成一个向量算全局范数再裁剪。
                        DETR/SparseR-CNN 系列（多学习率分组）推荐此模式，因为它保持
                        不同学习率组之间的相对梯度尺度不被 per-group 裁剪破坏。
    ...
    clip_type = GradientClipType(cfg.SOLVER.CLIP_GRADIENTS.CLIP_TYPE)

    if clip_type == GradientClipType.FULL_MODEL:
        # 全模型裁剪：复用 _generate_optimizer_class_with_gradient_clipping 的 global_clipper 钩子
        # （detectron2 原作者为 DETR 预留的机制，见该函数 line 69-72）。
        OptimizerWithGradientClip = _generate_optimizer_class_with_gradient_clipping(
            optimizer_type, global_clipper=grad_clipper
        )
    else:
        # per-param / per-group 裁剪（传统 R-CNN 路径，detectron2 原生）。
        OptimizerWithGradientClip = _generate_optimizer_class_with_gradient_clipping(
...
def build_optimizer(cfg: CfgNode, model: torch.nn.Module) -> torch.optim.Optimizer:
    ...
    按 cfg.SOLVER.OPTIMIZER 选择优化器：
      - "SGD"  ：传统路径，用 get_default_optimizer_params 按模块类型分组（传统 R-CNN 用）。
      - "ADAMW"：SparseR-CNN 路径，逐参数分组，参数名含 "backbone" 的乘以
                 cfg.SOLVER.BACKBONE_MULTIPLIER（对齐原版 SparseR-CNN 的 build_optimizer）。
    """
    optimizer_type = cfg.SOLVER.OPTIMIZER
    if optimizer_type == "SGD":
        # 传统 R-CNN 路径：保持原有按模块类型分组逻辑不变。
        ...
        # SparseR-CNN 路径：逐参数分组，backbone 用单独学习率（对齐原版 train_net.py 的 build_optimizer）。
