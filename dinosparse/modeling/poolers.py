...
def assign_boxes_to_levels(
    ...
    level_assignments = torch.floor(
        canonical_level + torch.log2(box_sizes / canonical_box_size + 1e-8)  # floor(4 + log2(sqrt(box_area) / 224))
    )
...
class ROIPooler(nn.Module):
    ...
    def forward(self, x: List[torch.Tensor], box_lists: List[Boxes]):
        ...
        # 旧版注释，未验证
        # 展开box_lists中每个元素，按batch中的图片顺序拼接，维度是[batch内所有图片总的box数量,5]，5表示图片在batch内索引和坐标
        pooler_fmt_boxes = convert_boxes_to_pooler_format(box_lists)

        if num_level_assignments == 1:  # 无FPN的情况
            return self.level_poolers[0](x[0], pooler_fmt_boxes)

        # 旧版注释，未验证
        # 为每个box分配FPN层级：level索引0，对应最大的画幅，即最小/浅的层
        # 展开box_lists中每个元素，按batch中的图片顺序拼接，维度是[batch内所有图片总的box数量]
        ...
        # 旧版注释，未验证，return按理讲应该大差不差
        # [batch内所有图片总的box数量,c,后两个维度是roi align输出形状如7*7]
        return output
